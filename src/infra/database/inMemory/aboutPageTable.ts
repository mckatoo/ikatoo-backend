...
type OptionalAboutPageProps = Partial<AboutPageProps>
...
export default {
  ...
  insert: (newPage: AboutPageProps) => {
    const { id } = newPage
    if (!!data.find(page => page.id === id)) throw new Error('The id already exists.')
    if (!id) newPage.id = randomUUID()

    data.push(newPage)
  },
  update: (id: string, newData: OptionalAboutPageProps) => {
    const oldData: AboutPageProps | undefined = data.find(page => page.id === id)
  ...
  delete: (id: string) => {
    if (!data.find(page => page.id === id)) throw new Error('The id does not exist.')

    data.splice(data.findIndex(page => page.id === id), 1)
  }
}